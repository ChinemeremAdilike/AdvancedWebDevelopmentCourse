...
let updateButton = null;     // ✅ ADDED (Phase3 bug fix)
let deleteButton = null;     // ✅ ADDED (Phase3 bug fix)
...
function isResourceDescriptionValid(value) {
  ...
  // ✅ FIXED — teacher’s original regex allowed < > / \ which is incorrect
  const allowedPattern = /^[a-zA-Z0-9äöåÄÖÅ .,!?-]+$/;
...
function attachResourceDescriptionValidation(input) {
  const update = () => {
    ...
    setInputVisualState(
      input,
      resourceDescriptionValid ? "valid" : "invalid"
    );
    setButtonEnabled(createButton, resourceNameValid && resourceDescriptionValid);
  };
...
const resourceDescriptionArea =
  createResourceDescriptionArea(resourceDescriptionCnt);
